...
Purpose: Universal hanging index of things to be done — the canonical gap between what the system IS and what it NEEDS TO BE. (855 items, 20 domains)
...
| DONE-330 | **ORGAN-IV directory dissection (exit interview post-mortem).** Full flattened-hierarchy report: 30 directories mapped across 3 structural classes (9 tracked submodules, 6 untracked core, 15 untracked contrib/external). 6 ontological domain cross-cuts (time, universals, ledgers, plans, markdown/research, cross-threshold). 12 skeletons catalogued across 5 severity tiers. Key findings: 21 directories invisible to git (local-only phantoms), 5,053 .md files (est. 1,200 organic), byte-identical agentkit duplicate, 3 governance-rules.json copies, 5 network-map.yaml copies, ceremony inflation (168+ files). .gitignore fixed to track DISSECTION.md. 10 new IRF items logged (CND-019 through CND-028). Plan: `.claude/plans/2026-04-04-organ-iv-directory-dissection.md`. | S-dissection | 2026-04-04 |
